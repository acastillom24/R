...
### a)	Perfile a los clientes según su frecuencia de compra: calcule la frecuencia de compra para cada cliente. 
...
freqCompra <- 
  dataGSS %>%  
  group_by(Segment, Customer.ID, Customer.Name, Order.ID) %>% 
  summarise(
    Sales = sum(Sales),
    Profit = sum(Profit)) %>% 
  group_by(Segment, Customer.ID, Customer.Name) %>% 
  summarise(
    NumCompras = n(), 
    Sales = sum(Sales),
    Profit = sum(Profit)) %>% 
