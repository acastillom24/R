# Info from:
# https://envejecimientoenred.wordpress.com/2015/02/18/piramides-de-poblacion-con-r/
library(showtext)
library(plotrix)
...
function (lx, rx, labels = NA, top.labels = c("Male", "Age", "Female"), 
          main = "", laxlab = NULL, raxlab = NULL, 
          unit = "%", lxcol, rxcol, gap = 1, space = 0.2, 
          ppmar = c(4, 2, 4, 2), labelcex = 1, add = FALSE, xlim, 
          show.values = FALSE, ndig = 1, do.first = NULL) 
{
  ...
  if (!add) {
    ...
    if (gap > 0) {
      ...
      else axis(2, at = 1:ncats * as.logical(rx + 1), labels = rep("", ncats), 
                pos = gap, tcl = -0.25)
      if (!is.null(lxdim)) 
        axis(4, at = 1:ncats, labels = rep("", ncats), pos = -gap, tcl = -0.25)
      else axis(4, at = 1:ncats * as.logical(lx + 1), labels = rep("", ncats), 
                pos = -gap, tcl = -0.25)
    }
    ...
    cat(xlim, "\n")
    mtext(top.labels, side = 3, line = 0, at = c(-(xlim[1] + gap)/2, 0, 
                                                 (xlim[2] + gap)/2), adj = 0.5, 
          cex = labelcex)
    if (length(unit) == 1) 
      mtext(unit, 1, 2.5, at = 0, cex = labelcex)
    else mtext(c(unit, unit), 1, 2.5, at = c(-(xlim[1] + gap)/2, 
                                             (xlim[2] + gap)/2), 
               cex = labelcex)
  }
